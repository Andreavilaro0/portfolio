...
export function CRTStatic() {
  ...
  useEffect(() => {
    ...
    let lastGlitch = 0
    let frameSkip = 0
    // FIX 16: Reuse the ImageData buffer — only reallocate when canvas dimensions change
    let imageData: ImageData | null = null

    const resize = () => {
      canvas.width = canvas.offsetWidth
      canvas.height = canvas.offsetHeight
      // Invalidate cached buffer so it is reallocated at the new size
      imageData = null
    }
    ...
    const draw = (timestamp: number) => {
      ...
      // Base static noise — reuse buffer, only reallocate on size change
