...
export function CircuitBackground() {
  ...
  useEffect(() => {
    ...
    function drawTrace(trace: Trace) {
      ...
      const glowColorFull = trace.glowColor + (baseAlpha * 3) + ')'

      // 1. Outer glow (wide, soft) — shadowBlur replaces ctx.filter blur(8px)
      ctx.save()
      ctx.shadowBlur = 12
      ctx.shadowColor = glowColorFull
      ctx.beginPath()
      ...
      // 2. Inner glow (medium) — shadowBlur replaces ctx.filter blur(3px)
      ctx.save()
      ctx.shadowBlur = 5
      ctx.shadowColor = trace.glowColor + (baseAlpha * 5) + ')'
      ctx.beginPath()
      ...
      // 4. Nodes at each completed corner — glowing dots — shadowBlur replaces ctx.filter blur(4px)
      for (let j = 0; j <= segIndex && j < totalPoints; j++) {
        ...
        ctx.save()
        ctx.shadowBlur = 6
        ctx.shadowColor = trace.glowColor + (baseAlpha * 6) + ')'
        ctx.beginPath()
      ...
      // 5. Moving pulse dot at the tip — shadowBlur replaces ctx.filter blur(3px)
      const tip = pathPoints[pathPoints.length - 1]
      ctx.save()
      ctx.shadowBlur = 5
      ctx.shadowColor = trace.glowColor + (baseAlpha * 8) + ')'
      ctx.beginPath()
